...
void loadGroundModel(vector<Vertex>& dst) {
	tinyobj::attrib_t attrib;
	std::vector<tinyobj::shape_t> shapes;
	std::vector<tinyobj::material_t> mats;
	string error;
...
void loadShipModel(vector<Vertex>& dst) {
	tinyobj::attrib_t attrib;
	std::vector<tinyobj::shape_t> shapes;
	std::vector<tinyobj::material_t> mats;
	string error;
...
void loadSkyboxModel(vector<Vertex>& dst) {
	tinyobj::attrib_t attrib;
	std::vector<tinyobj::shape_t> shapes;
	std::vector<tinyobj::material_t> mats;
	string error;
...
cst::ModelService::ModelService()
{
	...
	screenService.with([this](ComPtr<ID3D11Device> device) {
		vector<Vertex> quadModelVerts;
		loadSkyboxModel(quadModelVerts);
	...
	screenService.with([this](ComPtr<ID3D11Device> device) {
		...
		ComPtr<ID3D11Buffer> buffer;
		ThrowIfFailed(device->CreateBuffer(&bd, &sd, &buffer));
		_models.insert({ "Ground",{ buffer,quadModelVerts.size() } });

	});

	screenService.with([this](ComPtr<ID3D11Device> device) {
		vector<Vertex> quadModelVerts;
		loadShipModel(quadModelVerts);
		...
		ComPtr<ID3D11Buffer> buffer;
		ThrowIfFailed(device->CreateBuffer(&bd, &sd, &buffer));
		_models.insert({ "Ship",{ buffer,quadModelVerts.size() } });
